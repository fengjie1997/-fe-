  ...
  <el-collapse v-model="active" class="form-collapse">
    <el-collapse-item name="1">
      ...
      <el-form label-width="180px">
        <el-form-item :label="$t('taskPlan.mftpDown.mftpUpConfig')">
          <el-table v-col-width-limit :data="tableData" style="width: 90%" border>
            ...
            <el-table-column type="expand" label="#" show-overflow-tooltip>
              <template slot-scope="props">
                <el-form label-position="left" inline class="demo-table-expand">
                  ...
                  <el-form-item :label="$t('taskPlan.mftpDown.doSave')">
            ...
            <el-table-column prop="downloadFile" :label="$t('taskPlan.ftpDownload.downloadFile')" show-overflow-tooltip width="180" />
            <el-table-column prop="isSaveFile" :label="$t('taskPlan.mftpDown.doSave')" show-overflow-tooltip width="180" />
            ...
            <el-table-column :label="$t('common.actions')" fixed="right" min-width="85" show-overflow-tooltip>
              <template slot="header" slot-scope="scope">
                <el-tooltip :content="$t('common.add')">
                  <el-button icon="el-icon-plus" :name="scope" type="primary" @click="dialogVisible = true, update = false, title = $t('taskPlan.mftpDown.addConfig'), resetModel()" />
                </el-tooltip>
              </template>
              <template slot-scope="scope">
                <el-tooltip :content="$t('common.delete')">
                  <el-button icon="el-icon-delete" plain type="danger" :name="scope.row.id" @click="handleDel(scope.$index)" />
                </el-tooltip>
                <el-tooltip :content="$t('common.edit')">
                  <el-button icon="el-icon-edit" :name="scope.row.id" @click="title = $t('taskPlan.mftpDown.updateConfig'), handleUpdate(scope.row)" />
                </el-tooltip>
              </template>
            </el-table-column>
          </el-table>
      ...
      <el-dialog :title="title" :visible.sync="dialogVisible" width="50%" class="conf-form-dialog" :close-on-click-modal="false" append-to-body>
        <el-form ref="form" :rules="rules" :model="confForm" label-width="100px" label-position="right" size="mini" class="conf-form">
          <el-row>
            <el-col :span="12">
              <el-form-item :label="$t('taskPlan.ftpDownload.downloadFile')" prop="downloadFile">
