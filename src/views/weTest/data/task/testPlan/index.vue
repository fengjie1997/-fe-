                ...
                <div>
                  ...
                  >
                    ...
                    <el-table-column :label="$t('common.actions')" align="center" width="200">
                      <template slot-scope="scope">
                        <!--                        <el-button v-waves :loading="listLoading" class="filter-item" type="primary" @click="handleSeeIssue(scope.row)">{{ '查看下发' }}</el-button>-->
...
export default {
  ...
  methods: {
    getList() {
      ...
      if (this.model.testPlanList !== undefined && this.model.testPlanInfo !== '' && this.model.testPlanInfo !== null) {
        this.testPlanList = []
        for (const i in this.model.list) {
          this.testPlanList.push(JSON.parse(JSON.stringify(this.model.list[i])))
        }
      }
      if (this.model.uuids.length === 1 && this.model.uuids[0].length === 0) {
        this.model.uuids = []
      }
